...
<ParamField path="output_variable_name" type="str" required>
    Variable name to store the fetched 2FA code. Available as `{variable_name[0]}` in subsequent
    actions.
</ParamField>
...
<ParamField path="totp_secret" type="str" required>
    Base32-encoded TOTP secret key. This is the text form of the QR code typically scanned by
    authenticator apps.
</ParamField>
