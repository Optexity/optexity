...
| `idempotency_key` | `string \| null` | `null` | no | Dedupe key, unique per API key. A repeat request with the same key returns the **existing** `task_id` (HTTP 202, `"idempotent_replay": true`) instead of creating a second task. Use it when you retry a submit after a timeout. |
