...
This guide assumes you have already completed the steps in [Installation](/docs/installation).
...
### Example
When you first log in to your account, you will see built-in example automations.
The first automation is to download a PDF. You can see this on your dashboard.
![Example automation](/images/example_workflow.png)

The name of the automation is "download_pdf_url". Use this as the `endpoint_name` in your request.
The input parameter is `pdf_url`:
```json
{
  ...
  "unique_parameter_names": ["pdf_url"]
}
```
...
![Task runs](/images/task_runs.png)
