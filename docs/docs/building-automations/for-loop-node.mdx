...
At loop start the runtime waits up to `locator_timeout` for the first match to attach, counts matches once, then iterates `0 .. count-1`. An empty match set runs zero iterations (same as an empty list variable).
...
| `locator_timeout` | `float` | `5.0` | Locator loops only: seconds to wait for the first match to attach before counting |
...
Locator loops only. `count()` does not auto-wait, and the sleep after an action returns as soon as the page has loaded — which is immediately for an SPA that re-renders without navigating. Counting straight away would see zero rows and skip the loop body silently, so a locator loop first waits up to `locator_timeout` (default 5s) for the first match to attach.

| Situation | Behavior |
|-----------|----------|
| Matches appear within `locator_timeout` | Loop runs over the count at that moment |
| No match within `locator_timeout` | Zero iterations, no error — an empty result table is a valid outcome |
...
Raise `locator_timeout` for slow grids; set it to `0` to count immediately when the rows are already on the page.
...
- Locator loops snapshot `count()` once at start; DOM changes mid-loop do not change how many iterations run. Set `max_iterations` as a guard and re-check state inside the loop rather than relying on the initial count
