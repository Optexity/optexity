...
| `{row}` / `{index_of(locator)}` | Current numeric index (`0`, `1`, …) |
...
- Use `{locator[index]}` for the current match and bare `{index}` (or `{index_of(locator)}`) for the numeric index
- When nesting locator loops, give each a distinct `index_variable_name` and reference `{locator[row]}` / `{locator[cell]}` so each level keeps its own match token; avoid `{index_of(locator)}` inside nested locator loops (it is not scoped per level)
