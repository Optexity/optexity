  ...
  "nodes": [
    {
      ...
      "interaction_action": {
        "click_element": {
          "command": "{row}.get_by_role(\"button\", name=\"Edit\")"
        }
      }
    }
  ],
...
At loop start the runtime counts matches once, then iterates `0 .. count-1`. An empty match set runs zero iterations (same as an empty list variable).
...
Bare `{index_variable_name}` expands to the Playwright command for the current match (`<locator>.nth(<N>)`), so you can pass it as a `command` or chain off it. Use `{index_of(locator)}` for the numeric index:

```
locator = get_by_role("row")  (3 matches)

Iteration 1: {row} → get_by_role("row").nth(0)
Iteration 2: {row} → get_by_role("row").nth(1)
Iteration 3: {row} → get_by_role("row").nth(2)
```

| Pattern | Expands to |
|---------|------------|
| `{row}` / `{index}` | Current match command (`<locator>.nth(<N>)`) |
| `{index_of(locator)}` | Current numeric index (`0`, `1`, …) |
| `{row}.get_by_role("button")` | Chained command on the current match |
...
- Locator loops snapshot `count()` once at start; DOM changes mid-loop do not change how many iterations run
- In locator mode, bare `{index}` is a locator command (not a number)—use `{index_of(locator)}` for the numeric index
