...
├── enable_browser_alerts   # Log native JS dialogs (alert/confirm/prompt) as they're auto-accepted
...
| `enable_browser_alerts` | `bool` | `false` | Log native JS dialogs (`alert`, `confirm`, `prompt`) as they're encountered. See [Browser Alerts](#browser-alerts) |
...
| `nodes` | `list[action_node \| for_loop_node \| if_else_node \| private_node]` | — | Ordered list of actions |
...
| `private_node` | Closed-source handler, runs only on Optexity's servers | [Private Node](/docs/building-automations/private-node) |
...
Native JS dialogs (`alert`, `confirm`, `prompt`) are always auto-accepted so they never block the automation. Set `enable_browser_alerts` to also log each dialog's type and message when one is triggered:
...
<Tip>
Turn this on while building or debugging an automation against a site that pops native dialogs, so you can see in the logs when and why one fired. Leave it off otherwise.
</Tip>
