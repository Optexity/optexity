...
```
Extraction produced null value(s) in variable 'pcp_required' and allow_none is False: [None]
```
...
```json
{
  ...
  "extraction_action": {
    ...
    "llm": {
      ...
      "extraction_instructions": "Extract the notice_text banner if one is displayed on the page. Return null if no banner is present. Do not guess.",
      "output_variable_names": ["notice_text"]
    }
  }
}
```
...
<Warning>
Only use `output_variable_names` when you need to reference the extracted value in a later action (e.g. `{pcp_required[0]}`). If you only need the value in the output data and won't reference it downstream, omit `output_variable_names` entirely — the null-check is skipped and the value is simply stored in `OutputData`.
</Warning>
...
| `output_variable_name` | `str \| None` | `None` | Variable name to store the extracted text. When omitted, the value is stored under an auto-generated key (`node{index}_output`) and `extraction_format` must contain exactly one field. |
| `extraction_format` | `dict` | Required | Must contain `output_variable_name` as a key (or exactly one field when `output_variable_name` is omitted) |
