...
When field labels are not known at authoring time, fetch values first with [Dynamic Form Mapping](/docs/action-types/dynamic-form-mapping) and pass `{form_values[0]}` into the agentic task prompt.
